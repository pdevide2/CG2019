Public Class Status
    ...
    Public Overrides Sub inicio()
        'MyBase.inicio()
        Me.PkString = False
        Me.Tabela = "CG_STATUS" 'Nome da tabela no SQL
        Me.Modulo = 5 'codigo do módulo 
    ...
    Public Overrides Sub Gravar(acao As Integer)
        ...
        End If
        'MyBase.Gravar(acao)
        Dim bll As New BLL.StatusBLL
        Dim objStatus As New DTO.Cg_status
        Try
            objStatus.Id_status = CInt(txtCodigo.Text)
            objStatus.Desc_status = txtDescricao.Text
            objStatus.User_ins = ACE_CODIGO
            objStatus.Data_ins = Hoje()
            objStatus.User_upd = ACE_CODIGO
            objStatus.Data_upd = Hoje()

            bll.GravarBLL(acao, objStatus)
