...
Public Class MovimentoEquipamentoQuarentena
    ...
    Public Sub Carregar()
        ...
        sql += " FROM VW_CG_MOVIMENTO_EQUIPAMENTO_QUARENTENA_ITEM "
        Me.Comando = sql
    ...
    Public Overrides Sub inicio()
        ...
        Me.PkString = False
        Me.Tabela = "CG_MOVIMENTO_EQUIPAMENTO_QUARENTENA" 'Nome da tabela no SQL
        Me.View = "VW_CG_MOVIMENTO_EQUIPAMENTO_QUARENTENA"
        Me.TabelaFilha = "CG_MOVIMENTO_EQUIPAMENTO_QUARENTENA_ITEM"
        Me.Modulo = 44 'codigo do módulo 
        ...
        Me.ObjModel = New DTO.Cg_movimento_equipamento_quarentena
    ...
    Public Overrides Sub Gravar(acao As Integer)
        ...
        Dim bllPai As New BLL.Movimento_equipamento_quarentenaBLL
        Dim objMovimentoEquipamentoQuarentena As New DTO.Cg_movimento_equipamento_quarentena

        Dim bllFilha As New BLL.Movimento_equipamento_quarentena_itemBLL
        Dim objMovimentoEquipamentoQuarentenaItem As New DTO.Cg_movimento_equipamento_quarentena_item

        Try
            objMovimentoEquipamentoQuarentena.Id_romaneio = CInt(txtCodigo.Text)
            objMovimentoEquipamentoQuarentena.Data_movto = CDate(txtDataMovto.Text)
            objMovimentoEquipamentoQuarentena.Id_loja_origem = CInt(txtIdLoja.Text)
            objMovimentoEquipamentoQuarentena.Id_loja_destino = CInt(txtIdLojaDestino.Text)
            objMovimentoEquipamentoQuarentena.Id_responsavel = CInt(txtIdResponsavel.Text)
            objMovimentoEquipamentoQuarentena.Tipo_op = IIf(flagAcao = Operacao.Novo, "I", "A")

            objMovimentoEquipamentoQuarentena.Qtd_recebida = 0 'Nesta tela entra tudo em Transito
            objMovimentoEquipamentoQuarentena.Qtd_total = dgvDados.RowCount 'Quantidade de itens no Grid

            'Dados de usuarios
            objMovimentoEquipamentoQuarentena.User_ins = ACE_CODIGO
            objMovimentoEquipamentoQuarentena.Data_ins = Hoje()
            objMovimentoEquipamentoQuarentena.User_upd = ACE_CODIGO
            objMovimentoEquipamentoQuarentena.Data_upd = Hoje()

            bllPai.GravarBLL(acao, objMovimentoEquipamentoQuarentena)
            ...
            sqlquery += "FROM CG_MOVIMENTO_EQUIPAMENTO_QUARENTENA_ITEM WHERE ID_ROMANEIO = " & objMovimentoEquipamentoQuarentena.Id_romaneio.ToString

            strItens = BLL.GlobalBLL.PesquisarFkListaBLL(sqlquery)

            'Percorre o array e exclui item a item 
            objMovimentoEquipamentoQuarentenaItem.Id_romaneio = objMovimentoEquipamentoQuarentena.Id_romaneio
            ...
            For Each row As DataGridViewRow In dgvDados.Rows
                ...
                If Not row.IsNewRow Then
                    ...
                    objMovimentoEquipamentoQuarentenaItem.Id_equipamento = CInt(row.Cells(1).Value)
                    objMovimentoEquipamentoQuarentenaItem.Serie = row.Cells(2).Value
                    objMovimentoEquipamentoQuarentenaItem.Qtd = CInt(row.Cells(5).Value)
                    objMovimentoEquipamentoQuarentenaItem.Valor = Convert.ToDouble(Replace(row.Cells(6).Value, ".", ","))
                    objMovimentoEquipamentoQuarentenaItem.Id_romaneio = CInt(txtCodigo.Text)
                    objMovimentoEquipamentoQuarentenaItem.Unique_keyid = row.Cells(8).Value

                    'Operação de delete/insert
                    bllFilha.GravarBLL(Operacao.Novo, objMovimentoEquipamentoQuarentenaItem)
    ...
    Public Overrides Sub Alterar()
        ...
        If Verifica_Recebimento(CInt(txtCodigo.Text), "A") Then
            ...
            'Desabilita botão de Pesquisar Loja Destino - Loja 7 - QUARENTENA - FIXO
    ...
    Public Overrides Sub Incluir()
        ...
        If Me.KeyId > 0 Then
            ...
            Dim sqlquery As String
            sqlquery = "SELECT ID_LOJA, NOME FROM CG_LOJA WHERE dbo.CG_LOJA.ID_LOJA = 7"
    ...
    Private Sub btnAdiciona_Click_1(sender As Object, e As EventArgs) Handles btnAdiciona.Click
        If String.IsNullOrEmpty(txtIdLoja.Text) Or txtIdLoja.Text = "0" Then
            ...
            Exit Sub
        End If
        Dim frm As New WinCG.frmSelecionaEquipamento(CInt(txtCodigo.Text), CInt(txtIdLoja.Text), dgvDados, "spSelecionaEquipamentoMovimentoQUARENTENA")
        frm.ShowDialog()
    End Sub
    ...
    Private Function Verifica_Recebimento(ByVal _id_romaneio As Integer, ByVal _modo As String) As Boolean
        ...
        Try
            sql = "SELECT QTD_TOTAL, QTD_RECEBIDA FROM CG_MOVIMENTO_EQUIPAMENTO_QUARENTENA WHERE ID_ROMANEIO = {0}"
