...
Public Class MovimentoEquipamentoInativo
    ...
    Public Sub Carregar()
        ...
        sql += " FROM VW_CG_MOVIMENTO_EQUIPAMENTO_INATIVO_ITEM "
        Me.Comando = sql
    ...
    Public Overrides Sub inicio()
        ...
        Me.PkString = False
        Me.Tabela = "CG_MOVIMENTO_EQUIPAMENTO_INATIVO" 'Nome da tabela no SQL
        Me.View = "VW_CG_MOVIMENTO_EQUIPAMENTO_INATIVO"
        Me.TabelaFilha = "CG_MOVIMENTO_EQUIPAMENTO_INATIVO_ITEM"
        ...
        Me.ObjModel = New DTO.Cg_movimento_equipamento_inativo
    ...
    Public Overrides Sub Gravar(acao As Integer)
        ...
        Dim bllPai As New BLL.Movimento_equipamento_inativoBLL
        Dim objMovimentoEquipamentoInativo As New DTO.Cg_movimento_equipamento_inativo

        Dim bllFilha As New BLL.Movimento_equipamento_inativo_itemBLL
        Dim objMovimentoEquipamentoInativoItem As New DTO.Cg_movimento_equipamento_inativo_item

        Try
            objMovimentoEquipamentoInativo.Id_romaneio = CInt(txtCodigo.Text)
            objMovimentoEquipamentoInativo.Data_movto = CDate(txtDataMovto.Text)
            objMovimentoEquipamentoInativo.Id_loja_origem = CInt(txtIdLoja.Text)
            objMovimentoEquipamentoInativo.Id_loja_destino = CInt(txtIdLojaDestino.Text)
            objMovimentoEquipamentoInativo.Id_responsavel = CInt(txtIdResponsavel.Text)
            objMovimentoEquipamentoInativo.Tipo_op = IIf(flagAcao = Operacao.Novo, "I", "A")

            objMovimentoEquipamentoInativo.Qtd_recebida = 0 'Nesta tela entra tudo em Transito
            objMovimentoEquipamentoInativo.Qtd_total = dgvDados.RowCount 'Quantidade de itens no Grid

            'Dados de usuarios
            objMovimentoEquipamentoInativo.User_ins = ACE_CODIGO
            objMovimentoEquipamentoInativo.Data_ins = Hoje()
            objMovimentoEquipamentoInativo.User_upd = ACE_CODIGO
            objMovimentoEquipamentoInativo.Data_upd = Hoje()

            bllPai.GravarBLL(acao, objMovimentoEquipamentoInativo)
            ...
            sqlquery += "FROM CG_MOVIMENTO_EQUIPAMENTO_INATIVO_ITEM WHERE ID_ROMANEIO = " & objMovimentoEquipamentoInativo.Id_romaneio.ToString

            strItens = BLL.GlobalBLL.PesquisarFkListaBLL(sqlquery)

            'Percorre o array e exclui item a item 
            objMovimentoEquipamentoInativoItem.Id_romaneio = objMovimentoEquipamentoInativo.Id_romaneio
            ...
            For Each row As DataGridViewRow In dgvDados.Rows
                ...
                If Not row.IsNewRow Then
                    ...
                    objMovimentoEquipamentoInativoItem.Id_equipamento = CInt(row.Cells(1).Value)
                    objMovimentoEquipamentoInativoItem.Serie = row.Cells(2).Value
                    objMovimentoEquipamentoInativoItem.Qtd = CInt(row.Cells(5).Value)
                    objMovimentoEquipamentoInativoItem.Valor = Convert.ToDouble(Replace(row.Cells(6).Value, ".", ","))
                    objMovimentoEquipamentoInativoItem.Id_romaneio = CInt(txtCodigo.Text)
                    objMovimentoEquipamentoInativoItem.Unique_keyid = row.Cells(8).Value

                    'Operação de delete/insert
                    bllFilha.GravarBLL(Operacao.Novo, objMovimentoEquipamentoInativoItem)
    ...
    Private Sub btnAdiciona_Click_1(sender As Object, e As EventArgs) Handles btnAdiciona.Click
        If String.IsNullOrEmpty(txtIdLoja.Text) Or txtIdLoja.Text = "0" Then
            ...
            Exit Sub
        End If
        Dim frm As New WinCG.frmSelecionaEquipamento(CInt(txtCodigo.Text), CInt(txtIdLoja.Text), dgvDados, "spSelecionaEquipamentoMovimentoInativo")
        frm.ShowDialog()
    End Sub
    ...
    Private Function Verifica_Recebimento(ByVal _id_romaneio As Integer, ByVal _modo As String) As Boolean
        ...
        Try
            sql = "SELECT QTD_TOTAL, QTD_RECEBIDA FROM CG_MOVIMENTO_EQUIPAMENTO_INATIVO WHERE ID_ROMANEIO = {0}"
