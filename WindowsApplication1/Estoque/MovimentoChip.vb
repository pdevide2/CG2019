Public Class MovimentoChip
    ...
    Public Overrides Sub inicio()
        ...
        Me.PkString = False
        Me.Tabela = "CG_MOVIMENTO_CHIP" 'Nome da tabela no SQL
        Me.View = "VW_CG_MOVIMENTO_CHIP"
        Me.TabelaFilha = "CG_MOVIMENTO_CHIP_ITEM"
        Me.Modulo = 19 'codigo do módulo 
        ...
        Me.ObjModel = New DTO.Cg_movimento_chip
    ...
    Public Overrides Sub Gravar(acao As Integer)
        ...
        Dim bllPai As New BLL.Movimento_chipBLL
        Dim objMovimentoChip As New DTO.Cg_movimento_chip

        Dim bllFilha As New BLL.Movimento_chip_itemBLL
        Dim objMovimentoChipItem As New DTO.Cg_movimento_chip_item

        Try
            objMovimentoChip.Id_romaneio = CInt(txtCodigo.Text)
            objMovimentoChip.Data_movto = CDate(txtDataMovto.Text)
            objMovimentoChip.Id_loja_origem = CInt(txtIdLoja.Text)
            objMovimentoChip.Id_loja_destino = CInt(txtIdLojaDestino.Text)
            objMovimentoChip.Id_responsavel = CInt(txtIdResponsavel.Text)
            objMovimentoChip.Tipo_op = IIf(flagAcao = Operacao.Novo, "I", "A")

            objMovimentoChip.Qtd_recebida = 0 'Nesta tela entra tudo em Transito
            objMovimentoChip.Qtd_total = dgvDados.RowCount 'Quantidade de itens no Grid

            'Dados de usuarios
            objMovimentoChip.User_ins = ACE_CODIGO
            objMovimentoChip.Data_ins = Hoje()
            objMovimentoChip.User_upd = ACE_CODIGO
            objMovimentoChip.Data_upd = Hoje()

            bllPai.GravarBLL(acao, objMovimentoChip)
            ...
            sqlquery += "FROM CG_MOVIMENTO_CHIP_ITEM WHERE ID_ROMANEIO = " & objMovimentoChip.Id_romaneio.ToString

            strItens = BLL.GlobalBLL.PesquisarFkListaBLL(sqlquery)

            'Percorre o array e exclui item a item 
            objMovimentoChipItem.Id_romaneio = objMovimentoChip.Id_romaneio
            ...
            For Each row As DataGridViewRow In dgvDados.Rows
                ...
                If Not row.IsNewRow Then
                    ...
                    objMovimentoChipItem.Id_chip = CInt(row.Cells(1).Value)
                    objMovimentoChipItem.Simid = row.Cells(2).Value
                    objMovimentoChipItem.Qtd = CInt(row.Cells(5).Value)
                    objMovimentoChipItem.Valor = Convert.ToDouble(Replace(row.Cells(6).Value, ".", ","))
                    objMovimentoChipItem.Id_romaneio = CInt(txtCodigo.Text)
                    objMovimentoChipItem.Unique_keyid = row.Cells(8).Value

                    'Operação de delete/insert
                    bllFilha.GravarBLL(Operacao.Novo, objMovimentoChipItem)
    ...
    Private Sub btnAdiciona_Click_1(sender As Object, e As EventArgs) Handles btnAdiciona.Click
        If String.IsNullOrEmpty(txtIdLoja.Text) Or txtIdLoja.Text = "0" Then
            ...
            Exit Sub
        End If
        Dim frm As New WinCG.frmSelecionaChip(CInt(txtCodigo.Text), CInt(txtIdLoja.Text), dgvDados, "spSelecionaChipMovimento")
        frm.ShowDialog()
    End Sub
    ...
    Private Function Verifica_Recebimento(ByVal _id_romaneio As Integer, ByVal _modo As String) As Boolean
        ...
        Try
            sql = "SELECT QTD_TOTAL, QTD_RECEBIDA FROM CG_MOVIMENTO_CHIP WHERE ID_ROMANEIO = {0}"
