Public Class TipoServico
    ...
    Public Overrides Sub inicio()
        'MyBase.inicio()
        Me.PkString = False
        Me.Tabela = "CG_TIPO_SERVICO" 'Nome da tabela no SQL
        Me.Modulo = 10 'codigo do módulo 
    ...
    Public Overrides Sub Carrega_Controles_Pesquisa()
        ...
        If Me.KeyId <> -1 Then
            ...
            txtDescricao.Text = Me.LinhaGrid.Cells("DESC_TIPO_SERVICO").Value.ToString
        End If
    End Sub
    Public Overrides Sub Gravar(acao As Integer)
        ...
        End If
        'MyBase.Gravar(acao)
        Dim bll As New BLL.Tipo_servicoBLL
        Dim objTipoServico As New DTO.Cg_tipo_servico
        Try
            objTipoServico.Id_tipo_servico = CInt(txtCodigo.Text)
            objTipoServico.Desc_tipo_servico = txtDescricao.Text
            objTipoServico.User_ins = ACE_CODIGO
            objTipoServico.Data_ins = Hoje()
            objTipoServico.User_upd = ACE_CODIGO
            objTipoServico.Data_upd = Hoje()
            objTipoServico.Id_empresa = Publico.Id_empresa

            bll.GravarBLL(acao, objTipoServico)
