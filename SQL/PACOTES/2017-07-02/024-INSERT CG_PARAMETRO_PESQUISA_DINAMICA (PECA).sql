...
SELECT 
	...
	CASE 
		...
		WHEN sc.name like 'NOME_P%' THEN 1
		ELSE 0
...
inner join 
	sysobjects so 
		on so.id = sc.id
where so.type = 'V' 
--and LEFT(OBJECT_NAME(sc.id),2)='VW'
and OBJECT_NAME(sc.id) = 'VW_CG_PECA'
order by 1, 3 
GO

UPDATE CG_PARAMETRO_PESQUISA_DINAMICA
SET CHAVE = 1 WHERE COLUNA = 'ID_PECA' AND TABELA = 'VW_CG_PECA'
GO

--INSERT INTO CG_PARAMETRO_PESQUISA_DINAMICA
--(TABELA, COLUNA, IDCOLUNA, ORDERCOL, CHAVE, TIPODADO, COLUNA_FILTRO, APELIDO_COLUNA, MOSTRA_COLUNA)
--VALUES
--('VW_CG_PECA', 'ID_CONTROLE', 1, 1, 0, 'String', 0, 'ID_CONTROLE', 1);
--GO